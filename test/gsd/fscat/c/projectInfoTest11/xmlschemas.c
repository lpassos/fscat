...
#define PARSE_2_DIGITS(num,cur,invalid) if ((cur[0] < '0') || (cur[0] > '9') || (cur[1] < '0') || (cur[1] > '9')) invalid = 1; else num = (cur[0] - '0') * 10 + (cur[1] - '0'); cur += 2;
#define PARSE_FLOAT(num,cur,invalid) PARSE_2_DIGITS(num, cur, invalid); if (!invalid && (*cur == '.')) { double mult = 1; cur++; if ((*cur < '0') || (*cur > '9')) invalid = 1; while ((*cur >= '0') && (*cur <= '9')) { mult /= 10; num += (*cur - '0') * mult; cur++; } }
static int
...
#define PARSE_DIGITS(num,cur,num_type) if ((*cur < '0') || (*cur > '9')) num_type = -1; else while ((*cur >= '0') && (*cur <= '9')) { num = num * 10 + (*cur - '0'); cur++; }
#define PARSE_NUM(num,cur,num_type) num = 0; PARSE_DIGITS(num, cur, num_type); if (!num_type && (*cur == '.')) { double mult = 1; cur++; if ((*cur < '0') || (*cur > '9')) num_type = -1; else num_type = 1; while ((*cur >= '0') && (*cur <= '9')) { mult /= 10; num += (*cur - '0') * mult; cur++; } }
static int
...
#define RETURN_TYPE_IF_VALID(t) if (IS_TZO_CHAR(*cur)) { ret = _xmlSchemaParseTimeZone(&(dt->value.date), &cur); if (ret == 0) { if (*cur != 0) goto error; dt->type = t; goto done; } }
    if (dateTime == NULL)
 return -1;
    if (collapse)
...
#define TIME_TO_NUMBER(dt) ((double)((dt->value.date.hour * SECS_PER_HOUR) + (dt->value.date.min * SECS_PER_MIN) + (dt->value.date.tzo * SECS_PER_MIN)) + dt->value.date.sec)
static int
